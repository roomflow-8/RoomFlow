...
### 예약 시스템 동시성 제어
1. **적용 필요성**
- 회의실: 동일 시간 슬롯에 여러 사용자가 동시에 예약 요청시 동일 슬롯 중복 예약 발생
- 비품
  - 동일한 날짜 비슷한 시간대에 여러 사용자가 동일 자원에 대해 동시 예약 요청시
       => 동일한 비품에 대해 초과 예약 발생 가능 및 데이터 정합성 깨짐

2. **기존 방식 문제점**
- 기존 비관적 락 적용
  - DB에서 데이터를 조회한 뒤, 재고 계산 및 총 재고수량 수정하도록 구현
  - 재고 계산 과정에서 동일 트랜잭션 내에서 아직 반영되지 않은 이전 값으로 재고가 계산되어 초과 예약 발생


3. **적용**
- Redis 기반 분산락 (Redisson MultiLock) 적용
- 시간슬롯, 비품 단위 Lock key 생성 후 정렬하여 동시성 제어 수행
- 여러 개의 자원을 동시에 락 할 때, 서로 다른 순서로 Lock을 잡으면 데드락이 발생할 수 있어 정렬이 필요
...
### 배포
- 과정
  - GitHub Acations main/develop브랜치 
  - push 시에 자동 실행 
  - GitHub Secrets에 저장되어있는 YML 파일 주입 
  - Gradle 빌드 >> JAR 생성 
  - Docker 이미지 빌드 & DockerHub push 
  - EC2배포 - SSH 접속 // 최신이미지 pull 
  - 컨테이너 재시작
